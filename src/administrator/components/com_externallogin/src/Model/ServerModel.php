...
class ServerModel extends AdminModel
{
    ...
    protected function loadFormData()
    {
        ...
        $data = $app->getUserState('com_externallogin.edit.server.data', []);

        if (empty($data)) {
            $data = parent::getItem();
            if (is_array($data->params) && isset($data->params['data'])) {
                $registry = new Registry($data->params['data']);
                $data->params = $registry->toArray();
            } elseif ($data->params instanceof Registry) {

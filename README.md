...
- **CAS has no standard equivalent of OIDC's `email_verified` claim, so its verification check is opt-in.** Since `username_xpath` defaults to the identity's email (mirroring the OIDC default above), the CAS plugin has the same self-asserted-email risk — but unlike OIDC, the CAS protocol defines no standard attribute set at all; whatever shows up in `<cas:attributes>` is entirely up to how the specific CAS-emitting server is configured, so there's no fixed attribute name to check by default. Set the optional `email_verified_xpath` field (left empty by default — no behavior change) to an XPath 1.0 expression that resolves to a boolean; login is denied only on an explicit `false`, so a misconfigured expression or a server that never released the attribute fails open rather than locking accounts out. Wrap the condition as `boolean(not(<attribute>) or <condition>)` so a response that omits the attribute entirely isn't misread as an explicit false (XPath's empty-node-set comparisons are always false) — only a present attribute holding a literal false denies. Cookbook, depending on how your identity provider exposes verification status:

  | Provider shape | XPath |
  | --- | --- |
  | Boolean-ish attribute (e.g. `emailVerified=true`, as configured for this repo's devcontainer demo — see `.devcontainer/keycloak/import/demo-realm.json`) | `boolean(not(cas:attributes/cas:emailVerified) or cas:attributes/cas:emailVerified = 'true')` |
  | Single sentinel text value (e.g. `status=confirm`) | `boolean(not(cas:attributes/cas:status) or cas:attributes/cas:status = 'confirm')` |
  ...
  | A multi-valued list of verified addresses instead of a single flag | `boolean(not(cas:attributes/cas:verified_email) or cas:attributes/cas:verified_email = cas:attributes/cas:email)` (XPath 1.0 node-set `=` node-set is an existential comparison — true if *any* value in the left set string-equals *any* value in the right set) |
